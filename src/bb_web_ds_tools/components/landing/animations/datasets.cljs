...
(def phase-duration 2000)
(def display-time 1500)
(def transition-time 500)

(defn- calculate-alpha [local-t]
  (let [progress (if (> local-t display-time)
                   (/ (- local-t display-time) transition-time)
                   0)]
    (- 1.0 progress)))

(defn- draw-data-table [ctx w h _t local-t]
  (let [alpha (calculate-alpha local-t)
        rows 4 cols 4
        cell-w 20 cell-h 12
...
(defn- draw-filter-map [ctx w h t local-t]
  (let [progress (if (> local-t display-time)
                   (/ (- local-t display-time) transition-time)
                   0)
...
(defn- draw-transposed-table [ctx w h _t local-t]
  (let [progress (if (> local-t display-time)
                   (/ (- local-t display-time) transition-time)
                   0)
        alpha-in (min 1.0 (/ local-t transition-time))
...
(defn- draw-vertical-bars [ctx w h t local-t]
  (let [alpha-in (min 1.0 (/ local-t transition-time))
        cols 5
        bar-w 15 gap 5
...
(defn draw-datasets [ctx w h t]
  ...
  (let [cycle 8000
    ...
    (cond
      ...
      (draw-data-table ctx w h t phase)

      ;; Phase 2: Filter/Map (2000 - 4000ms)
      (< phase (* 2 phase-duration))
      (draw-filter-map ctx w h t (- phase phase-duration))

      ;; Phase 3: Transposed Data Table (4000 - 6000ms)
      (< phase (* 3 phase-duration))
      (draw-transposed-table ctx w h t (- phase (* 2 phase-duration)))

      ;; Phase 4: Vertical Bars (6000 - 8000ms)
      :else
      (draw-vertical-bars ctx w h t (- phase (* 3 phase-duration))))))
