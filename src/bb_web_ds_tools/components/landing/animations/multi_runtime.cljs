...
(def code-lines ["(->> ms (map :x)..."
                 "sum([m['x'] for m...])"
                 "Reduce(+, Filter..."])
(def colors ["#dca3a3"  ;; red-ish (Clojure)
...
(defn- draw-typing [ctx _w _h _t phase cx cy]
...
(defn- draw-converging [ctx _w _h t phase cx cy]
...
(defn- draw-processing [ctx _w _h t _phase cx cy]
  (let [box-size 80]
    ;; Machine Pulse
...
(defn draw-multi-runtime [ctx w h t]
  ...
  (let [cx (/ w 2)
    ...
    (cond
      ...
      (< phase 4000)
      (draw-typing ctx w h t phase cx cy)

      ;; Phase 2: Converging to Machine (4000 - 5500ms)
      (< phase 5500)
      (draw-converging ctx w h t phase cx cy)

      ;; Phase 3: Machine Processing (5500 - 8000ms)
      :else
      (draw-processing ctx w h t phase cx cy))))
