...
(defn- draw-gear [ctx t x y r teeth speed color]
  (.save ctx)
  (.translate ctx x y)
  (.rotate ctx (* t speed))
...
(defn draw-settings [ctx w h t]
  ...
  (let [cx (/ w 2)
        cy (/ h 2)]
    (draw-gear ctx t cx cy 40 8 0.001 (:portal.colors/text zenburn))
    (draw-gear ctx t (+ cx 50) (+ cy 50) 25 6 -0.002 (:portal.colors/namespace zenburn))))
