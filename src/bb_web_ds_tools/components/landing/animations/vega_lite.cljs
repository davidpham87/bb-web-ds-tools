...
(defn- draw-code-snippet [ctx _w _h _t phase cx cy]
  (let [code-snippet "{:mark :bar :encoding ...}"]
...
(defn- draw-bar-chart [ctx _w _h _t chart-phase cx cy]
  (let [transition 500
        progress (min 1.0 (/ chart-phase transition))
...
(defn- draw-line-chart [ctx w _h _t chart-phase cy]
  (let [transition 500
        local-t (- chart-phase 2000)
...
(defn- draw-scatter-plot [ctx w h _t chart-phase cx cy]
  (let [transition 500
        local-t (- chart-phase 4000)
        progress (min 1.0 (/ local-t transition))]
...
(defn draw-vega-lite [ctx w h t]
  ...
  (let [cx (/ w 2)
    ...
    (cond
      ...
      (< phase 2000)
      (draw-code-snippet ctx w h t phase cx cy)

      ;; Phase 2: Full Screen Charts Sequence (2000 - 8000ms)
      :else
      (draw-charts ctx w h t phase cx cy))))
