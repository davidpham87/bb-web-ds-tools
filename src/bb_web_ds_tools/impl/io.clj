...
(defmethod parse-string :csv [_ text]
  (let [data (csv/read-csv text)
        header (first data)
        rows (rest data)]
    (mapv #(zipmap header %) rows)))

(defmethod parse-string :json [_ text]
  (json/read-str text :key-fn keyword))

(defmethod parse-string :edn [_ text]
  (edn/read-string text))

(defmethod parse-string :yaml [_ text]
  (yaml/parse-string text))

(defmulti write-string (fn [format _] (keyword format)))

(defmethod write-string :csv [_ data]
  (let [sw (java.io.StringWriter.)]
    (cond
      ;; If map (columnar), convert to row-maps first
      (map? data)
      (let [row-maps (datasets/transform data :columnar :row-maps)
...
(defmethod write-string :json [_ data]
  (json/write-str data {:indent true}))

(defmethod write-string :edn [_ data]
  (with-out-str (pprint/pprint data)))

(defmethod write-string :yaml [_ data]
