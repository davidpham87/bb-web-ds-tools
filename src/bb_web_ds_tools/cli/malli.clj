...
;; Need to ensure dependencies for malli are available in bb.edn if running via
;; bb
;; or just assume the user runs this with correct classpath.

(def cli-specs
  {:infer {:max-enum {:desc "Max distinct values for enum inference",
                      :ref "<int>",
                      :alias :e,
                      :coerce :int,
                      :default 10,
                      :default-desc "10"},
           :file
           {:desc "Input file (stdin if omitted)", :ref "<file>", :alias :i}},
   :generate {:samples {:desc "Number of samples to generate",
                        :ref "<int>",
                        :alias :s,
                        :coerce :int,
                        :default 1,
              ...
              :file {:desc "Schema input file (stdin if omitted)",
                     ...
                     :alias :i}},
   :validate
   {:schema {:desc "Schema file", :ref "<file>", :alias :s, :require true},
    :file {:desc "Data file (stdin if omitted)", :ref "<file>", :alias :i}}})
...
(defn infer
  ...
  (let [input-str (read-input opts)
        ;; Try to parse as EDN first, then fallback or fail. For CLI, we
        ;; might expect valid EDN or JSON if we implement JSON parsing here
        ;; or reuse component logic.
