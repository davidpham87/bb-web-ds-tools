  ...
  (testing "New Logic: Enum if Unique <= 200 AND (Unique < 10% OR MaxLen < 60)"
    ...
    (testing "Scenario 4: Medium Dataset, Unique < 200, but > 10%, Short Strings"
      ...
      (let [unique-vals (map #(str "V" %) (range 50))
            ...
            res (sut/infer-schema data)]
        (is (= {:success true
                :schema [:map [:a [:enum "V0" "V1" "V10" "V11" "V12" "V13" "V14" "V15" "V16" "V17" "V18" "V19" "V2" "V20" "V21" "V22" "V23" "V24" "V25" "V26" "V27" "V28" "V29" "V3" "V30" "V31" "V32" "V33" "V34" "V35" "V36" "V37" "V38" "V39" "V4" "V40" "V41" "V42" "V43" "V44" "V45" "V46" "V47" "V48" "V49" "V5" "V6" "V7" "V8" "V9"]]]}
               res))))
    ...
    (testing "Scenario 5: Long Strings (Description)"
      ...
      (let [long-str (apply str (repeat 70 "X"))
            ...
            res (sut/infer-schema data)]
        (is (= {:success true
                :schema [:map [:a [:enum "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"]]]}
               res))))
