...
2.  **Start the development server:** Conect to localhost:8080 to see the app
    and browse. Use this with e2e testing to perform changes.

    ```bash
    npx shadow-cljs watch app &
    ```

3.  **Run tests:** To have have test running continuously on file change

    ```bash
    npx shadow-cljs watch text-node &
    ```

    Alternatively, conect to localhost:8081 to see the test running or failing
...
the
[official Re-frame documentation](https://day8.github.io/re-frame/getting-started/).
...
- **State Location:** For performance reasons, component state should be stored
  in a dedicated path within the `app-db`. Avoid storing component state at the
  root level.

- **Subscription Layering:** Create layered subscriptions to avoid unnecessary
  re-computations. A base subscription can select a component's root state, and
  more specific subscriptions can derive from it.
...
- Use `malli.generator` to create generative tests for your components and event
  handlers. This is particularly useful for ensuring your code can handle a wide
