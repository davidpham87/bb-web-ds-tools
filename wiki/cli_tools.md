...
Ensure you have `bb` (Babashka) installed on your path. The entry point is the `src/dstools.clj` script.
...
Manage and convert tabular data between formats (CSV, JSON, EDN, Markdown) and structures (Row Maps, Columnar, Row Arrays).
...
*   `-o, --out <file>`: Output file path (defaults to stdout; infers extension if input provided).
*   `-S, --input-struct <struct>`: Input structure (`row-maps`, `columnar`, `rows`).
*   `-s, --output-struct <struct>`: Output structure (`row-maps`, `columnar`, `rows`).
...
Compile [HoneySQL](https://github.com/seancorfield/honeysql) data structures into standard SQL.

**Command:** `convert`

**Features:**
*   **Compilation**: Transforms `.edn` files containing Clojure maps into `.sql` files.
...
Work with [Malli](https://github.com/metosin/malli) schemas for validation, inference, and generation.
...
**Options (Common):**
*   `-i, --file <file>`: Input file (data for `infer`/`validate`, schema for `generate`).
